...
if ($form->is_submitted()) {
    $data = $form->get_data();
    // Stores in DB as there may be many many much entries to just pass it by url.
    $cache = $form->store_options($data, $report->orphanedfiles);
    $filestodelete = unserialize($cache->filestodelete);
    $confirmurl = new moodle_url('/admin/tool/filecheck/deleteorphans.php', array(
        'confirmdelete' => true,
        'cacheid' => $cache->id));
    echo html_writer::tag('p', get_string('confirm_delete', 'tool_filecheck'));
    $i = 0;
    ...
    echo $OUTPUT->single_button($confirmurl, get_string('delete'));
